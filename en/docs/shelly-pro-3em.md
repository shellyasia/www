...
The Shelly Pro 3EM is a DIN rail-mountable three-phase energy meter. Enhanced with full `gen2` firmware flexibility and LAN connectivity, it offers professional integrators expanded options for end-customer solutions. The device can operate standalone on a local LAN or Wi-Fi network, or be managed via cloud-based home automation services using MQTT, HTTP, and WebSocket protocols. All inbound connections support TLS encryption.

The device reports real-time measurements of accumulated energy, as well as instantaneous voltage, current, active power, and apparent power per phase. Data is stored in non-volatile memory and retained for up to 60 days at 1-minute intervals.

It includes a built-in real-time clock to maintain accurate time even if the connection to an SNTP server is lost.

Shelly Pro 3EM can be remotely accessed, configured, and monitored by users, and can also communicate with automation systems—provided both are within the same network infrastructure.

An embedded web interface allows users to monitor and control the device, adjust settings, and view operational data.

> **⚠ NOTICE!** The device does not include a built-in relay. Contactor control is enabled via a Shelly Pro Add-on connected to the Shelly Pro 3EM.
...
- Current transformer (CT) inputs for accurate current sensing  
...
> \* Phase sequence error detection monitors zero-crossing events in phase voltages. The expected sequence is Phase A → Phase B → Phase C. If the sequence is A → C → B, a `phase_sequence` error is reported when the feature is enabled.  
>  
> \*\* Calibration requires at least 500 W load per channel.  
>  
> \*\*\* When total load across all three channels drops below 30 VA per channel, measured power is displayed but no energy is accumulated. A `No load threshold` notification appears in the web interface and mobile app.
...
- 4 line inputs via screw terminals: 3 **L** (Phase A, B, C), 1 **N** (Neutral)  
- 4 CT inputs: 3 for measuring **L** currents (A, B, C), 1 for **N** current measurement  

### Ethernet Port

- 1 × RJ45 connector  

> **⚠ CAUTION!** Always disconnect or connect the LAN cable only when the device is powered off. Ensure the connector’s user-accessible parts are non-metallic to prevent electric shock during plugging/unplugging.

### Add-on Interface

- Proprietary serial interface for connecting Shelly Pro Add-ons  
...
- One tactile dome button  
  - **Short press**: Toggle the state of the relay in the attached Shelly Pro 3EM Switch Add-on  
...
- **LED indicators**  
  - **Power**: Red light when power is connected  
  - **Wi-Fi status** (varies):  
    - Blue: AP mode  
  ...
  - **LAN**: Green light when Ethernet is connected  
  - **Count**: Red LED flashes at a frequency proportional to energy flow through the circuit  
...
| Channel-to-channel calibration min. load | 500 W per channel |
| No load threshold | 30 VA per channel |
| Measurement data storage | ≥60 days at 1-minute resolution |
| Data export | CSV (for PQ values), JSON via RPC |
| **Radio** | |
| RF band | 2400 – 2495 MHz |
| Max. RF power | <20 dBm |
| Wi-Fi protocol | 802.11 b/g/n |
| Wi-Fi range | Up to 30 m indoors / 50 m outdoors (depends on environment) |
| Bluetooth protocol | 4.2 |
| Bluetooth range | Up to 10 m indoors / 30 m outdoors (depends on environment) |
| **MCU** | |
| CPU | ESP32-D0WDQ6 |
| Flash memory | 16 MB |
...
| L | Monophase live wire (110–240 V) |
| N | Neutral wire |
...
- [This Device API Reference](https://shelly-api-docs.shelly.cloud/gen2/Devices/Gen2/ShellyPro3EM)  
