...
Shelly 1 Gen3 是一款紧凑型智能开关，具备无源触点功能，可通过智能手机、平板电脑、PC 或家庭自动化系统远程控制电器。该设备可在本地 Wi-Fi 网络中独立运行，也可通过基于云的家庭自动化服务进行管理。相比前代产品，它配备了升级的处理器和更大的内存，性能更优。

该设备支持在互联网连接状态下实现远程访问、控制和监控，前提是设备始终连接至 Wi-Fi 路由器并保持联网。

适用于标准电气墙盒内改造安装，可置于电源插座后方、灯开关后方或空间受限的位置。

Shelly 1 Gen3 内置网页界面，用于设备监控、配置与管理。

## 主要特性

- **Wi-Fi 连接：** 连接到家庭 Wi-Fi 网络，实现远程监控与控制。
- **智能家居集成：** 兼容 Google Assistant、Amazon Alexa 和 Samsung SmartThings，支持语音控制与自动化。
- **本地与云端控制：** 可在本地网络中独立运行，也可集成到基于云的平台。
- **蓝牙连接：** 支持蓝牙及 BLE 网关功能，便于初始设置阶段使用。
- **用户友好界面：** 配备物理复位按钮，支持手动操作。
- **升级的处理器与内存：** 更强硬件带来更佳性能与响应速度。
- **嵌入式网页界面：** 通过浏览器即可访问，实现全面设备控制与参数调整。
- **无线协议支持：** 支持 Wi-Fi（802.11 b/g/n）和蓝牙 4.2，具备明确的室内外覆盖范围。
- **干接点切换：** 专为安全切换低电压设备设计。
- **BLE 网关功能：** 作为 Shelly BLU 设备与更广泛 Shelly 生态系统之间的桥梁，接收蓝牙信号并转发至云端或本地设备。
- **物联网设备 Wi-Fi 范围扩展器：** 通过接收、放大并重新传输信号，扩展 Wi-Fi 覆盖范围至更大区域。
...
- **定时调度：** 支持基于日期、时间、工作日、小时、分钟和秒的复杂自动化计划。
- **虚拟组件：** [虚拟动态组件文档](https://shelly-api-docs.shelly.cloud/gen2/DynamicComponents/Virtual/)

## 应用场景

- **远程电器控制：** 使用手机、平板电脑、PC 或家庭自动化系统开启/关闭电器。
- **互联网连接便利性：** 只要设备连接至 Wi-Fi 网络，即可从任何有互联网接入的位置远程控制。
- **家庭自动化：** 自动化电器的通断周期，提升舒适度与能源效率。
...
> **注意：** LED 状态按列出顺序优先级排序，后续状态将覆盖先前状态。
...
| **电气参数** | 供电电压 | 110–240 V~<br>24–48 V⎓<br>12 V⎓ |
| | 功耗 | < 1.2 W |
...
- [此设备](https://shelly-api-docs.shelly.cloud/gen2/Devices/Gen3/Shelly1G3)  
...
[Shelly 1 Gen3 多语言打印版用户与安全手册](https://kb.shelly.cloud/__attachments/391315459/Shelly%201%20Gen3%20multilingual%20printed%20user%20and%20safety%20guide.pdf?inst-v=06e25fb6-1df6-4585-801d-931808676f21)

- [使用与安全手册（俄语）](../knowledge-base/shelly-1-gen3-1)
