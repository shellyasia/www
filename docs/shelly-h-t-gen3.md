...
- **大尺寸电子墨水图形显示屏：** 高对比度、低功耗显示屏，实时显示温度和湿度信息。
...
- **蓝牙连接：** 设置过程中可用，用于设备配置与诊断。
- **用户友好界面：** 配备复位按钮（位于背面盖板后），便于手动操作与故障排查。
- **电池供电模式：** 在电池模式下，设备每分钟唤醒一次进行测量。若温差 ≥0.2 °C 或湿度变化 ≥3%，则更新屏幕并上报数据；若两小时内未触发阈值上报，则强制向云端发送状态。
- **USB 供电模式：** 通过 USB 供电时，设备每 5 分钟唤醒一次，执行测量、更新显示，并无条件上报数据。
- **环境规格：** 工作温度范围为 0 °C 至 40 °C（32 °F 至 105 °F）；推荐湿度范围：30% 至 70% RH。
- **电源选项：** 支持 4 节 AA 碱性电池（不包含）或 USB Type-C 连接供电。
- **无线连接：** 支持 Wi-Fi（802.11 b/g/n）和蓝牙 4.2，室内有效距离达 30 米（100 英尺），室外可达 50 米（160 英尺）。
- **微控制器单元（MCU）：** ESP-Shelly-C38F，具备 8 MB 闪存。

这些特性使 Shelly H&T Gen3 非常适用于住宅、多户住宅、轻型商业场所、大学/学院等需要可靠、低功耗环境监测并支持智能家居集成的场景。
...
1. 每分钟唤醒一次执行测量。  
   ...
   - 默认上报阈值：≥0.5 °C 或 ≥5% 变化。较小变化不会触发上报。  
2. 若两小时内未触发基于阈值的上报，设备将无条件向 Shelly Cloud 及所有连接通道发送状态。  
3. 由于自发热影响，每次测量周期后需强制执行 5 分钟冷却期。在此期间设备保持休眠，不再唤醒。
...
| 重量 | 47 g / 1.66 oz |
| 安装方式 | 带底座放置于平整表面<br>使用螺丝壁挂安装（螺帽直径：5–7 mm / 0.20–0.28 in，最大螺纹直径：3 mm / 0.12 in）<br>双面泡沫胶贴 |
| 外壳材质 | 塑料 |
| 外壳颜色 | 白色<br><ul><li>摩卡色：C:0 M:12 Y:28 K:12</li><li>象牙白：C:3 M:4 Y:14 K:8</li><li>黑色：C:0 M:0 Y:0 K:100</li></ul> |
...
| 估算电池寿命 | 约 1 年（具体取决于电池类型、环境条件及网络活动情况） |
| USB 供电 | Type-C（电源适配器与线缆不含） |
...
- [此设备](https://shelly-api-docs.shelly.cloud/gen2/Devices/Gen3/ShellyHTG3)  
...
- [Shelly H&T Gen 3 磨砂黑 UK PSTI ACT 合规声明](https://kb.shelly.cloud/__attachments/266174494/Shelly%20H&T%20Gen%203%20Matte%20Black%20UK%20PSTI%20ACT%20Statement%20of%20compliance.pdf?inst-v=06e25fb6-1df6-4585-801d-931808676f21)  
- [Shelly H&T Gen 3 磨砂白 UK PSTI ACT 合规声明](https://kb.shelly.cloud/__attachments/266174494/Shelly%20H&T%20Gen%203%20Matte%20White%20UK%20PSTI%20ACT%20Statement%20of%20compliance.pdf?inst-v=06e25fb6-1df6-4585-801d-931808676f21)  
