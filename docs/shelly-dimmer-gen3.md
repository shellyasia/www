...
Shelly Dimmer Gen3 是一款单通道智能调光控制器，可通过智能手机、平板电脑、PC 或家庭自动化系统远程控制可调光灯具和调光驱动器。该设备可在本地 Wi-Fi 网络中独立运行，也可集成至基于云的家庭自动化服务。只要设备保持联网状态，即可从任何具备互联网连接的位置进行远程访问、控制和监控。设备内置网页界面，用于配置、监控和调整设置。它作为尾沿调光开关工作，支持无中性线使用。

## 主要功能

- **无需中性线：** 可在无中性线情况下运行。
- **旁路要求：** 无中性线且负载低于 20 W 时需启用旁路，以消除灯关闭时的残余光。
- **灯具分组：** 支持将多个灯具分组，但同一组内的所有灯具必须具有相同的功耗（同型号、同品牌）。
- **蓝牙：** 符合蓝牙标准，支持与其他蓝牙设备进行短距离数据交换。
- **BLE 网关：** 实现 BLE 与 Wi-Fi 设备之间的通信。
- **Wi-Fi：** 连接无线网络。
- **Wi-Fi 范围扩展器：** 重复并扩展 Wi-Fi 信号覆盖范围。
- **脚本功能：** 支持通过脚本创建自定义自动化场景。
- **基础定时计划：** 支持每周定时计划和常规任务，包括亮度、过渡时间及调光信号翻转值设置。
- **高级定时计划：** 提供全年详细调度，时间调整范围从秒到月，支持亮度、过渡时间和翻转值配置。
- **自动开关计时器：** 可设置自动开启/关闭时间。
- **本地自动化：** 支持在本地 Wi-Fi 网络内实现自动化，包括亮度、过渡时间和翻转值调整。
- **Webhooks：** 支持轻量级事件驱动自动化，可与外部设备联动。
- **兼容性：** 完全兼容第三方家庭自动化平台。
- **无需网关：** 可通过 Shelly Smart Control 或第三方系统实现本地或远程使用。
- **活动日志：** 记录设备事件的详细历史。
- **KVS（键值存储）：** 提供持久化的键值对存储。
- **安全保护：** 包含过流、过压和过功率保护。这些保护功能在未使用中性线时可能无法正常工作。
- **电量测量：**
  - 电压（V）、电流（A）和功率（W）仅在使用中性线时可测量。
  - 未使用中性线时无法测量功耗。
- **夜间模式：** 可在夜间时段设定预设亮度。
- **过渡时间：** 控制开启时从 0% 到 100% 和关闭时从 100% 到 0% 的调光速度。
- **切换开启时最小亮度：** 当当前亮度低于阈值时，开启时应用的最小亮度（%）。默认值：3%。在计划或动作中显式定义过渡时不会生效。
- **最小/最大亮度：** 调整调光范围，实现更精细的输出亮度控制。
- **开关/按钮输入模式：** 支持灵活的输入控制方式：
  - 单按钮调光
  - 双按钮调光
- **按钮渐变速率：** 控制按住按钮时亮度变化的速度。默认值：3x。
- **按钮预设：** 定义双击触发的亮度状态。默认值：100%。
- **自动调光校准：** 确保全范围调光平滑顺畅。应在初始设置、出厂重置或更换负载后执行：
  - 新设备或出厂重置后，首次切换（通过物理开关/按钮、移动 App、本地 Web UI 或 RPC 命令）即启动校准。
  - 手动启动：同时长按两个按钮 5 秒。校准过程最长持续 2 分钟。
    - 成功：灯亮 10 秒后熄灭。
    - 失败：灯闪烁 5 次（1 秒开 / 1 秒关）。
- **通过开关/按钮执行出厂重置：** 上电后 60 秒内连续切换 5 次以触发出厂重置。
- **虚拟组件：** 用户创建的动态组件，设备上不存在实体硬件。
- **KNX：** 支持 KNXnet/IP 通信。

## 使用场景

- 即使没有中性线，也能改造现有照明系统，保留原有开关和布线。
- 精确控制可调光灯具的亮度，营造理想氛围。
- 创建智能定时计划，全天自动调节照明。
- 使用智能调度同步灯光与日出日落时间。
- 在夜间启用夜间模式，降低亮度。
- 与第三方平台（如 Home Assistant、Google Home、Alexa、SmartThings）集成。
- 通过单次或双次按键实现灯光的调亮或调暗。
- 通过 Shelly Motion 检测到运动时自动触发灯光，提升安全性。
...
| 无中性线且无旁路时最小负载 | 20 W |
| 调光类型 | 尾沿调光 |
| 外部保护 | 16 A，B 或 C 型脱扣特性，6 kA 分断能力，能量限制等级 3 |
| 短路保护 | 是 |
...
| 射频频段 | 2412 – 2472 MHz |
| 最大射频功率 | < 20 dBm |
| 传输范围 | 室内最高 30 米 / 室外最高 50 米<br>（受当地环境影响） |

| **蓝牙** | |
| 协议 | Bluetooth 4.2 |
| 射频频段 | 2402 – 2480 MHz |
| 最大射频功率 | < 4 dBm |
| 传输范围 | 室内最高 10 米 / 室外最高 30 米<br>（受当地环境影响） |
...
| **固件功能** | |
| 定时计划 | 20 个 |
| Webhooks（URL 操作） | 20 个，每条最多支持 5 个 URL |
| 脚本功能 | 支持 |
| MQTT | 支持 |
| 高级定时计划 | 支持 |
| KVS（键值存储） | 支持 |
...
- [使用与安全指南（俄语版）](../knowledge-base/shelly-dimmer-gen3-1)
